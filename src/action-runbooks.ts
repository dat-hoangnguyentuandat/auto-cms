...
export async function writeActionRunbook(stage: Stage, run: RunState, cmsRoot: string, runDir: string) {
  ...
  const workflow = stage === 'THEME_BUILD'
    ...
    : `Run the deterministic audit first:\n\n    node dist/cli.js qa --run ${run.runId} --base-url <PUBLIC_BASE_URL>\n\nThe generated reports/qa/qa-gate.json is authoritative and fingerprints the theme source plus evidence. Read its findings and fix every Critical/High issue inside theme ownership, then rerun QA after the final edit. Perform only the pending mutable-data fixtures listed in qa-draft.json. Merge real fixture evidence into submissions/qa-report.json and include qa-gate.json plus every deterministic evidence path. Manual claims cannot override the gate.`;
