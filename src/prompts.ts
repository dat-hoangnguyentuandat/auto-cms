...
const stageInstructions: Record<Exclude<Stage, 'ERP_READ' | 'PACKAGE' | 'FINAL_REPORT'>, (run: RunState, cms: string, runDir: string) => string> = {
  ...
  THEME_QA: (run, cms, dir) => `Run Auto CMS deterministic QA and treat ${dir}\\reports\\qa\\qa-gate.json as authoritative. Fix failures within theme ownership and rerun after the final source edit. Complete only pending mutable fixtures. Write ${dir}\\submissions\\qa-report.json containing {"passed":true,"critical":0,"high":0,"checks":[...],"evidence":[...]} and include qa-gate.json plus every gate evidence path. Manual reports cannot override a failed or stale gate.`,
};
